...
const Signup = () => {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      // Step 1: Fetch all doctors and check if the email exists
      const response = await axios.get("http://localhost:8080/api/employees/getAllEmployee");

      // Check if email exists in the list of frontdesk 
      const doctorExists = response.data.some(employee => employee.email === email && employee.role == 'frontdesk');

      if (doctorExists) {
        // Step 2: Proceed with signup if email is found
        const doctorData = { email, password };
        const signupResponse = await axios.post("http://localhost:8080/api/frontDeskRegisters/saveFrontDeskRegister", doctorData);

        if (signupResponse.status === 200) {
          setPopup({ type: "success", message: "Front Desk Details Saved Successfully"  });
        ...
        setPopup({ type: "error", message: "Email not registered as an Doctor ." });
      }
    } catch (error) {
      if (error.response) {
