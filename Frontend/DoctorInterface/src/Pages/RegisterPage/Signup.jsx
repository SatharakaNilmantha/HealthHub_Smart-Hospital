...
const Signup = () => {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      // Check if email exists in the list of doctors 
      const doctorExists = response.data.some(employee => employee.email === email);

      if (doctorExists) {
        // Step 2: Proceed with signup if email is found
        const doctorData = { email, password };
        const signupResponse = await axios.post("http://localhost:8080/api/registerDoctors/saveDoctor", doctorData);

        if (signupResponse.status === 200) {
          setPopup({ type: "success", message: "Doctor Details Saved Successfully"  });
